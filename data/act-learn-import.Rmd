...
exp_df <- data.frame(
    ...
    experimental_condition = assignments$answers$experimental_condition
)
```

```{r}
exp_df %>% 
    group_by(experimental_condition, order) %>% 
    summarise(n())
```
...
```{r comment=NA}
write.csv(x = df_final, file = "../../data_processed/act-learn-prior-manip.csv", row.names=F)
```
