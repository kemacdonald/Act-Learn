...
```{r}
df_exp1 <- df %>% filter(block == 1 | block == 2, condition != "YY", framing_condition == "none")
```
...
qplot(x=condition, y=mean_accuracy, data=ms_exp1,
...
block_plot_exp1 <- qplot(x=block, y=mean_accuracy, data=ms_block_exp1, color = condition,
...
block_plot2_exp1 <- qplot(x=block_factor, y=mean_accuracy, 
...
block_plot3_exp1 <- qplot(x=block_factor, y=mean_accuracy, 
...
qplot(x=dim_of_interest, y=other_dim, data = df_sampling_exp1,
...
```{r}
qplot(x=condition, y=mean_samp_dist, data=ms_sampling_exp1,
      geom="bar", stat="identity", fill = condition, position = "dodge") + 
...
```{r}
a_exp1 <- qplot(x=mean_samp_dist, y=mean_accuracy_active_block, data = filter(ss_all_exp1, block == 1)) +
...
b_exp1 <- qplot(x=mean_samp_dist, y=mean_accuracy_active_block, data = filter(ss_all_exp1, block == 2)) +
    geom_smooth(method="lm") +
    facet_grid(condition~category_type) +
    xlab("Mean Sample Distance") +
    ylab("Mean Accuracy") +
...
TODO: fix import scrip to track match/mismatch condition.

```{r}
df_exp2 %>% group_by(framing_condition, category_typeorder) %>% 
    summarise(n())
```