...
```{r}
df_rep <- df %>% filter(experiment == "replication")
```
...
df_rep %>%
    ...
    summarise(mean_correct = mean(correct), 
              ci_high = ci.high(correct),
              ci_low = ci.low(correct)) %>%
    ggplot(aes(x = conf_level, y = mean_correct, col = condition, group = condition)) +
...
### Relationship between evidence selection and test over time
...
```{r}
df_sequence %<>% mutate(order_rule_based = ifelse(order == "order1" & category_type == "Rule-Based", "Size",
