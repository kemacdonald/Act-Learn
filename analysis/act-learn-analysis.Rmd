...
ss <- inner_join(ss, select(ss_include, subids, include), by = "subids")

# filter out subjects that are +/- 3sd
df <- df %>% filter(include == 1)
ss <- ss %>% filter(include == 1)
```
...
df %>%
    group_by(gender) %>%
    summarise(n_distinct(subids))

# age
df %>% summarise(floor(median(age)))
df %>% summarise(min(age), max(age))
...
ms_block <- df %>%
    filter(trial_type == "test") %>%
    group_by(condition, block, odb_scale) %>%
    summarise(mean_accuracy = mean(correct),
              ci_high = ci.high(correct),
              ci_low = ci.low(correct))
```
...
qplot(x=block, y=mean_accuracy, data=ms_block, color = condition,
      geom=c("point", "line"), stat="identity", facets=.~odb_scale) + 
    geom_linerange(aes(ymin=mean_accuracy - ci_low, 
        ymax=mean_accuracy + ci_high), 
        width = .05, size=0.6) + 
    scale_colour_grey(start = 0, end = .5) +
