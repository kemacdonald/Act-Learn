...
library(dplyr)
library(lubridate)
setwd("../../production-results")
#setwd("../../sandbox-results")
```
...
```{r comment=NA}
exp_df <- data.frame(workerid = assignments$WorkerId, 
                     ...
                     scale_factor_orientation = assignments$answers$scale_factor_orienation,
...
```{r comment=NA}
sub_exp_df <- merge(sub_df, exp_df, by="workerid")
```
...
df_final$approval_time <- parse_date_time(gsub("[[:alpha:]]"," ", 
                                               df_final$approval_time),
                                          "Ymd hms")
# get list of subs sorted by approval time
df_remove <- df_final %>%
    filter(condition == "receptive", odb_scale == "radius_scale") %>%
    select(workerid, approval_time) %>%
    distinct() %>%
    arrange(approval_time) 

# remove the final 5 participants
df_remove <- df_remove[13:17,]

# convert to list
df_remove <- df_remove$workerid
```

Now we filter the data set, removing those subjects we flagged.

```{r filter extra subs}
df_final <- df_final %>%
    filter(!(workerid %in% df_remove))
```
...
```{r comment=NA}
setwd("../ACT-LEARN_GIT/data/")
write.csv(df_final_clean, file = "act-learn-processed-data.csv", row.names=F)
```
